...
# --- Create directories for caching and output figures
...
def report_solution(sol, config_name, airplane, op_point, mass_props, mass_props_TOGW, LD_cruise, sink_rate, aero):
    """A helper function to print outputs and generate plots for a solved configuration."""
    ...
    if make_plots:
        # ##################################################
        # # Consolidated Plotting (Corrected)
        # ##################################################

        # --- Generate aerodynamic polars for plotting
        ...
        # --- Create main figure and layout using GridSpec
        fig = plt.figure(figsize=(24, 16))
        fig.suptitle(f"{config_name.replace('_', ' ')}: Full Report", fontsize=24, y=0.98)
        gs = fig.add_gridspec(2, 2, width_ratios=[1, 1.2])

        # --- Left Side: 3-View and 3D plot
        gs_left = gs[:, 0].subgridspec(2, 2)
        ax_top = fig.add_subplot(gs_left[0, 0], projection='3d');
        ax_top.set_title("Top View")
        ax_front = fig.add_subplot(gs_left[1, 0], projection='3d');
        ax_front.set_title("Front View")
        ax_side = fig.add_subplot(gs_left[0, 1], projection='3d');
        ax_side.set_title("Side View")
        ax_3d = fig.add_subplot(gs_left[1, 1], projection='3d');
        ax_3d.set_title("3D Perspective")

        # Draw the airplane on each axis
        airplane.draw(ax=ax_top, show=False)
        airplane.draw(ax=ax_front, show=False)
        airplane.draw(ax=ax_side, show=False)
        airplane.draw(ax=ax_3d, show=False)

        # Set the camera views for orthographic projections
        ax_top.view_init(elev=90, azim=-90);
        ax_top.set_axis_off()
        ax_front.view_init(elev=0, azim=-90);
        ax_front.set_axis_off()
        ax_side.view_init(elev=0, azim=-180);
        ax_side.set_axis_off()

        # --- Top Right: Mass Budget Pie Chart
        ax_pie = fig.add_subplot(gs[0, 1])
        name_remaps = {k: k.replace("_", " ").title() for k in mass_props.keys()}
        mass_props_to_plot = {k: v for k, v in sol(mass_props).items() if v.mass > 1e-9}
        if "ballast" in mass_props_to_plot and mass_props_to_plot["ballast"].mass < 1e-6:
            del mass_props_to_plot["ballast"]

        plt.sca(ax_pie)  # Set current axis for pretty_plots
        p.pie(
            values=[v.mass for v in mass_props_to_plot.values()],
            names=[name_remaps.get(n, n) for n in mass_props_to_plot.keys()],
            center_text=f"$\\bf{{{config_name.replace('_', ' ')} Mass Budget}}$\nTOGW: {s(mass_props_TOGW.mass * 1e3):.2f} g",
        ...
        # --- Bottom Right: Aerodynamic Polars
        gs_aero = gs[1, 1].subgridspec(2, 2, wspace=0.25, hspace=0.4)
        ax_cl = fig.add_subplot(gs_aero[0, 0])
        ax_cd = fig.add_subplot(gs_aero[0, 1])
        ax_cm = fig.add_subplot(gs_aero[1, 0])
        ax_ld = fig.add_subplot(gs_aero[1, 1])
        ...
        ax_ld.set_xlabel(r"Angle of Attack $\alpha$ [deg]");
        ax_ld.set_ylabel(r"Lift/Drag Ratio $L/D$")
...
def optimize_h_tail():
    ...
    wing = asb.Wing(name="Main Wing", symmetric=True, xsecs=[
        ...
        asb.WingXSec(xyz_le=[-0.25 * wing_root_chord, wing_span / 2 * wing_center_span_fraction, 0],
                     chord=wing_root_chord, airfoil=airfoils["ag13"]), asb.WingXSec(
    ...
    h_tail = asb.Wing(name="Horizontal Stabilizer", symmetric=True,
                      xsecs=[asb.WingXSec(xyz_le=[0, 0, 0], chord=h_tail_root_chord, airfoil=airfoils["naca0008"]),
                             asb.WingXSec(xyz_le=[0.02, h_tail_span / 2, 0], chord=h_tail_root_chord * 0.8,
                                          airfoil=airfoils["naca0008"])]).translate([x_tail, 0, 0])

    v_tail_height = 0.12
    v_tail_root_chord = 0.09
    v_tail = asb.Wing(name="Vertical Stabilizer", symmetric=False,
                      xsecs=[asb.WingXSec(xyz_le=[0, 0, 0], chord=v_tail_root_chord, airfoil=airfoils["naca0008"]),
                             asb.WingXSec(xyz_le=[0.03, 0, v_tail_height], chord=v_tail_root_chord * 0.7,
    ...
    fuselage = asb.Fuselage(name="Fuse",
                            xsecs=[asb.FuselageXSec(xyz_c=[x_nose, 0, 0], height=1.5 * u.inch, width=2.0 * u.inch),
    ...
    mass_props['wing'] = asb.mass_properties_from_radius_of_gyration(mass=wing.volume() * density, x_cg=0,
                                                                     z_cg=(0.03591) * (np.sind(
                                                                         wing_dihedral_angle_deg) / np.sind(11)) * (
                                                                                  wing_span / 1))
    total_tail_volume = h_tail.volume() + v_tail_right.volume() + v_tail_left.volume();
    mass_props["tail_surfaces"] = asb.mass_properties_from_radius_of_gyration(mass=total_tail_volume * 80,
    ...
    mass_props["propeller_band"] = asb.mass_properties_from_radius_of_gyration(mass=0.06e-3,
                                                                               x_cg=mass_props["propeller"].x_cg)
    mass_props["flight_computer"] = asb.mass_properties_from_radius_of_gyration(mass=4.30e-3,
    ...
    mass_props["boom"] = asb.mass_properties_from_radius_of_gyration(mass=7.0e-3 * (boom_length / 826e-3),
                                                                     x_cg=(x_pod_end + x_tail) / 2)
    mass_props["ballast"] = asb.MassProperties(mass=opti.variable(init_guess=0, lower_bound=0),
    ...
    opti.subject_to([aero["L"] >= 9.81 * mass_props_TOGW.mass, aero["Cm"] == 0, static_margin == 0.08, LD_cruise == LD,
    ...
    opti.subject_to(h_tail.area() * tail_moment_arm / (wing.area() * wing.mean_aerodynamic_chord()) > 0.4)
    total_vtail_area = v_tail_right.area() + v_tail_left.area();
    opti.subject_to(total_vtail_area * tail_moment_arm / (wing.area() * wing.span()) > 0.02)
    opti.subject_to([x_nose < -0.25 * wing_root_chord - 0.5 * u.inch, x_tail - x_nose < 0.826])
...
def optimize_conventional_tail():
    ...
    v_tail = asb.Wing(name="Vertical Stabilizer", symmetric=False,
                      xsecs=[asb.WingXSec(xyz_le=[0, 0, 0], chord=v_tail_root_chord, airfoil=airfoils["naca0008"]),
                             asb.WingXSec(xyz_le=[0.03, 0, v_tail_height], chord=v_tail_root_chord * 0.7,
                                          airfoil=airfoils["naca0008"])]).translate([x_tail + 0.01, 0, 0])
    ...
    tail_moment_arm = h_tail.aerodynamic_center(chord_fraction=0.25)[0] - mass_props_TOGW.xyz_cg[0]
    opti.subject_to(h_tail.area() * tail_moment_arm / (wing.area() * wing.mean_aerodynamic_chord()) > 0.4)
    opti.subject_to(v_tail.area() * tail_moment_arm / (wing.area() * wing.span()) > 0.02)
    opti.subject_to([x_nose < -0.25 * wing_root_chord - 0.5 * u.inch, x_tail - x_nose < 0.826])
...
def optimize_v_tail():
    ...
    vtail = asb.Wing(name="V-Tail", symmetric=True,
                     xsecs=[asb.WingXSec(xyz_le=[0, 0, 0], chord=v_tail_root_chord, airfoil=airfoils["naca0008"]),
                            asb.WingXSec(
                                xyz_le=[0.02, v_tail_span / 2, (v_tail_span / 2) * np.tand(vtail_dihedral_deg)],
                                chord=v_tail_root_chord * 0.8, airfoil=airfoils["naca0008"])]).translate([x_tail, 0, 0])
    ...
    mass_props['wing'] = asb.mass_properties_from_radius_of_gyration(mass=wing.volume() * density, x_cg=0,
                                                                     z_cg=(0.03591) * (np.sind(
                                                                         wing_dihedral_angle_deg) / np.sind(11)) * (
                                                                                  wing_span / 1))
    mass_props["tail_surfaces"] = asb.mass_properties_from_radius_of_gyration(mass=vtail.volume() * 80,
                                                                              x_cg=x_tail + 0.50 * v_tail_root_chord)
    mass_props["linkages"] = asb.MassProperties(mass=1e-3, x_cg=(x_nose + x_tail) / 2)
    mass_props["motor"] = asb.mass_properties_from_radius_of_gyration(mass=4.49e-3, x_cg=x_nose - 0.3 * u.inch)
    mass_props["motor_bolts"] = asb.mass_properties_from_radius_of_gyration(mass=4 * 0.075e-3, x_cg=x_nose)
    mass_props["propeller"] = asb.mass_properties_from_radius_of_gyration(mass=1.54e-3, x_cg=x_nose - 0.7 * u.inch);
    ...
    opti.subject_to(projected_h_tail_area * tail_moment_arm / (wing.area() * wing.mean_aerodynamic_chord()) > 0.4)
    projected_v_tail_area = vtail.area() * np.cosd(vtail_dihedral_deg);
    opti.subject_to(projected_v_tail_area * tail_moment_arm / (wing.area() * wing.span()) > 0.02)
    opti.subject_to([x_nose < -0.25 * wing_root_chord - 0.5 * u.inch, x_tail - x_nose < 0.826])
...
def optimize_reverse_v_tail_twin_boom():
    ...
    reverse_v_tail = asb.Wing(name="Reverse V-Tail", symmetric=True, xsecs=[
        asb.WingXSec(xyz_le=[0, 0, 0], chord=v_tail_root_chord, airfoil=airfoils["naca0008"]), asb.WingXSec(
            xyz_le=[(v_tail_span / 2) * np.tand(v_tail_sweep), v_tail_span / 2,
                    -(v_tail_span / 2) * np.tand(v_tail_anhedral_deg)], chord=v_tail_root_chord * v_tail_taper,
            airfoil=airfoils["naca0008"])]).translate([x_tail, 0, tail_root_height])
    ...
    boom_radius = 6e-3 / 2
    boom_right = asb.Fuselage(name="Right Boom",
                              xsecs=[asb.FuselageXSec(xyz_c=[boom_end_x, boom_end_y, 0], radius=boom_radius),
                                     asb.FuselageXSec(xyz_c=[boom_start_x, boom_start_y, 0], radius=boom_radius)])
    boom_left = asb.Fuselage(name="Left Boom",
                             xsecs=[asb.FuselageXSec(xyz_c=[boom_end_x, -boom_end_y, 0], radius=boom_radius),
                                    asb.FuselageXSec(xyz_c=[boom_start_x, -boom_start_y, 0], radius=boom_radius)])
    ...
    mass_props['wing'] = asb.mass_properties_from_radius_of_gyration(mass=wing.volume() * density, x_cg=0,
                                                                     z_cg=(0.03591) * (np.sind(
                                                                         wing_dihedral_angle_deg) / np.sind(11)) * (
                                                                                  wing_span / 1))
    mass_props["tail_surfaces"] = asb.mass_properties_from_radius_of_gyration(mass=reverse_v_tail.volume() * 80,
                                                                              x_cg=x_tail + 0.50 * v_tail_root_chord)
    mass_props["linkages"] = asb.MassProperties(mass=1e-3, x_cg=(x_nose + x_tail) / 2)
    mass_props["motor"] = asb.mass_properties_from_radius_of_gyration(mass=4.49e-3, x_cg=x_nose - 0.3 * u.inch);
    mass_props["motor_bolts"] = asb.mass_properties_from_radius_of_gyration(mass=4 * 0.075e-3, x_cg=x_nose);
    ...
    mass_props["center_boom"] = asb.mass_properties_from_radius_of_gyration(mass=7.0e-3 * (center_boom_length / 826e-3),
                                                                            x_cg=(x_pod_end + x_tail) / 2)
...
def optimize_flying_wing():
    ...
    wing_tip_twist_deg = opti.variable(init_guess=-3, lower_bound=-10, upper_bound=0)
    ...
    xyz_le_tip = np.array([xyz_le_break[0] + span_outer_panel * np.tand(wing_sweep_deg), span_tip,
                           xyz_le_break[2] + span_outer_panel * np.tand(10)])
    ...
    winglet_sweep = 15;
    winglet_cant = 10
    winglets = asb.Wing(name="Winglets", symmetric=True, xsecs=[
        asb.WingXSec(xyz_le=xyz_le_tip + np.array([chord_tip * 0.75, 0, 0]), chord=winglet_chord,
                     airfoil=airfoils["naca0008"]), asb.WingXSec(xyz_le=xyz_le_tip + np.array(
            [chord_tip * 0.75 + winglet_height * np.tand(winglet_sweep), winglet_height * np.sind(winglet_cant),
             winglet_height * np.cosd(winglet_cant)]), chord=winglet_chord * 0.7, airfoil=airfoils["naca0008"])])
    ...
    mass_props = {}
    density = 2 * u.lbm / u.foot ** 3;
    mass_props['wing'] = asb.mass_properties_from_radius_of_gyration(mass=wing.volume() * density,
                                                                     x_cg=wing.aerodynamic_center()[0],
                                                                     z_cg=wing.aerodynamic_center()[2])
    mass_props["winglets"] = asb.mass_properties_from_radius_of_gyration(mass=winglets.volume() * 80,
                                                                         x_cg=winglets.aerodynamic_center()[0],
                                                                         z_cg=winglets.aerodynamic_center()[2])
    x_pusher_prop_mount = 0.6 * wing_root_chord
    mass_props["motor"] = asb.mass_properties_from_radius_of_gyration(mass=4.49e-3,
                                                                      x_cg=x_pusher_prop_mount - 0.3 * u.inch)
    mass_props["motor_bolts"] = asb.mass_properties_from_radius_of_gyration(mass=4 * 0.075e-3, x_cg=x_pusher_prop_mount)
    mass_props["propeller"] = asb.mass_properties_from_radius_of_gyration(mass=1.54e-3,
                                                                          x_cg=x_pusher_prop_mount + 0.4 * u.inch);
    mass_props["propeller_band"] = asb.mass_properties_from_radius_of_gyration(mass=0.06e-3,
                                                                               x_cg=mass_props["propeller"].x_cg)
    ...
    mass_props["battery"] = asb.mass_properties_from_radius_of_gyration(mass=4.61e-3,
                                                                        x_cg=x_avionics_bay - 1.5 * u.inch)
    mass_props["ballast"] = asb.MassProperties(mass=opti.variable(init_guess=0, lower_bound=0),
                                               x_cg=opti.variable(init_guess=0, lower_bound=-0.25 * wing_root_chord,
                                                                  upper_bound=0.75 * wing_root_chord))
    ...
    # --- Objective and Constraints
    opti.minimize(sink_rate / 0.3 + mass_props_TOGW.mass / 0.100 * 0.1 + (mass_props["ballast"].x_cg / 1e3) ** 2)
    opti.subject_to([aero["L"] >= 9.81 * mass_props_TOGW.mass, aero["Cm"] == 0, static_margin == 0.05, LD_cruise == LD,
                     design_mass_TOGW == mass_props_TOGW.mass])
